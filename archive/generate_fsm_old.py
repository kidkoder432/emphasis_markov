...
def normalize(tpm):
    """
    Normalize each row of a TPM (transition probability matrix) so that it sums to 1.
    Rows that sum to (almost) zero remain zeros.
    """
    row_sums = tpm.sum(axis=1, keepdims=True)
    # avoid division by zero
    row_sums[row_sums < 1e-10] = 1
    return tpm / row_sums
...
def generate_single_phrase(
    ...
    for _ in range(max_steps):
        ...
        # Avoid Repetition
        if len(phrase_notes) > 1 and next_note == current_note:
...
def main_with_eval():
    ...
    if ENABLE_VISUALIZATION:
        if best_table:
            create_tpm_dashboard(best_table, swars_list)
        else:
            logger.warning("Skipping visualization: No data available.")
    else:
        logger.info("Skipping visualization as per configuration.")
